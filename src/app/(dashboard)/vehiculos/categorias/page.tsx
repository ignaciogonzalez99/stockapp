...
import { getVehicleCategories, createVehicleCategory, updateVehicleCategory, deleteVehicleCategory, type VehicleCategory } from './actions';
import { CategoryModal } from '@/components/CategoryModal';
import { DeleteConfirmationModal } from '@/components/DeleteConfirmationModal';

export default function VehicleCategoriasPage() {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
    const [categories, setCategories] = useState<VehicleCategory[]>([]);
    const [selectedCategory, setSelectedCategory] = useState<VehicleCategory | undefined>(undefined);
    const [categoryToDelete, setCategoryToDelete] = useState<VehicleCategory | undefined>(undefined);
    ...
    const handleDeleteClick = (category: VehicleCategory) => {
    ...
    const confirmDelete = async () => {
        if (categoryToDelete) {
            try {
                ...
                const updated = await getVehicleCategories();
    ...
    const handleSubmit = async (formData: FormData) => {
        if (selectedCategory) {
            await updateVehicleCategory(selectedCategory.id, formData);
        } else {
        ...
        <div className="flex h-full flex-col gap-6">
            <div className="flex items-center justify-between rounded-md bg-white p-4 shadow-sm shrink-0">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                    <span className="font-bold text-sidebar">Categorías de Vehículos</span>
                    <span>&gt;</span>
                    <span>Vehículos</span>
                    <span>&gt;</span>
            ...
            <CategoryModal
                ...
                title="+ Nueva categoría de vehículo"
            />
