...
import { getReparacionesByEstado, type Reparacion } from '../reparaciones/actions';
...
export default function OrdenesTrabajoPage() {
    ...
    const loadReparaciones = useCallback(async () => {
        // We can reuse the action from reparaciones to get active ones
        ...
        <div className="flex h-full flex-col gap-6">
            <div className="flex flex-col gap-4 rounded-md bg-white p-4 shadow-sm shrink-0 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                    <span className="font-bold text-green-600">📋 Órdenes de Trabajo</span>
                    <span>&gt;</span>
                    <span>Inicio</span>
                    <span>&gt;</span>
                        ...
                        <table className="w-full text-left text-sm text-gray-500">
                            ...
                            <tbody>
                                ...
                                {reparaciones.length === 0 && (
                                    <tr>
                                        <td colSpan={8} className="px-6 py-4 text-center">No hay órdenes de trabajo activas</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            {/* Fixed Dropdown Menu - RESTRICTED Actions */}
            {activeDropdownId !== null && (
                ...
                >
                    <div className="py-1">
                        <button
                            onClick={() => {
                                const rep = reparaciones.find(r => r.id === activeDropdownId);
                        ...
                        <button
                            onClick={() => {
                                const rep = reparaciones.find(r => r.id === activeDropdownId);
                                setSelectedReparacionForEstado(rep);
                                setIsEstadoModalOpen(true);
                                setActiveDropdownId(null);
                            }}
                            className="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                        >
                            <span className="mr-2">🔄</span> Cambiar estado
                        </button>
                    </div>
                </div>
            )}

            {/* Reparacion Info Modal - STRICTLY READ ONLY */}
            <ReparacionInfoModal
                isOpen={isModalOpen}
                onClose={() => {
