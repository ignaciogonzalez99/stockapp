'use client';

import { useState } from 'react';
import { createCliente, updateCliente, type Cliente } from '@/app/(dashboard)/clientes/actions';
import { BottomSheet } from '@/components/BottomSheet';

interface ClienteModalProps {
    isOpen: boolean;
    onClose: () => void;
    cliente?: Cliente;
}

export function ClienteModal({ isOpen, onClose, cliente }: ClienteModalProps) {
    const [isPending, setIsPending] = useState(false);
    const [docType, setDocType] = useState<'RUT' | 'CI'>(cliente?.docType ?? 'RUT');

    if (!isOpen) return null;

    async function handleSubmit(formData: FormData) {
        setIsPending(true);
        try {
            formData.set('docType', docType);
            if (cliente) {
                await updateCliente(cliente.id, formData);
            } else {
                await createCliente(formData);
            }
            onClose();
        } catch (error) {
            console.error('Failed to save cliente', error);
            alert('Failed to save cliente');
        } finally {
            setIsPending(false);
        }
    }

    return (
        <BottomSheet
            isOpen={isOpen}
            onClose={onClose}
            title={cliente ? 'Editar cliente' : 'Nuevo cliente'}
            maxWidthClass="max-w-4xl"
        >
            <form action={handleSubmit} className="bottom-sheet-form space-y-4">
                <div>
                    <label className="block text-sm font-bold text-zinc-300">
                        <span className="text-red-500">*</span> Nombre o razon social
                    </label>
                    <input
                        type="text"
                        name="name"
                        defaultValue={cliente?.name}
                        required
                        className="mt-1 block w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 shadow-sm focus:border-green-500 focus:ring-green-500"
                    />
                </div>

                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        <label className="block text-sm font-bold text-zinc-300">
                            <span className="text-red-500">*</span> Telefono
                        </label>
                        <input
                            type="text"
                            name="phone"
                            defaultValue={cliente?.phone}
                            required
                            placeholder="Ingrese telefono"
                            className="mt-1 block w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 shadow-sm focus:border-green-500 focus:ring-green-500"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-zinc-300">E-mail</label>
                        <input
                            type="email"
                            name="email"
                            defaultValue={cliente?.email}
                            className="mt-1 block w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 shadow-sm focus:border-green-500 focus:ring-green-500"
                        />
                    </div>
                </div>

                <div>
                    <label className="mb-2 block text-sm font-bold text-zinc-300">
                        <span className="text-red-500">*</span> Rut o Nro documento
                    </label>
                    <div className="mb-2 flex gap-4 text-zinc-300">
                        <label className="flex cursor-pointer items-center hover:text-slate-700">
                            <input
                                type="radio"
                                name="docTypeRadio"
                                value="RUT"
                                checked={docType === 'RUT'}
                                onChange={() => setDocType('RUT')}
                                className="mr-2"
                            />
                            RUT
                        </label>
                        <label className="flex cursor-pointer items-center hover:text-slate-700">
                            <input
                                type="radio"
                                name="docTypeRadio"
                                value="CI"
                                checked={docType === 'CI'}
                                onChange={() => setDocType('CI')}
                                className="mr-2"
                            />
                            CI
                        </label>
                    </div>
                    <input
                        type="text"
                        name="docNumber"
                        defaultValue={cliente?.docNumber}
                        required
                        className="mt-1 block w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 shadow-sm focus:border-green-500 focus:ring-green-500"
                    />
                </div>

                <div>
                    <label className="block text-sm font-bold text-zinc-300">Direccion</label>
                    <input
                        type="text"
                        name="address"
                        defaultValue={cliente?.address}
                        className="mt-1 block w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 shadow-sm focus:border-green-500 focus:ring-green-500"
                    />
                </div>

                <div className="text-xs font-medium text-red-500">
                    * Campos obligatorios
                </div>

                <div className="flex justify-center gap-2 pt-4">
                    <button
                        type="button"
                        onClick={onClose}
                        className="flex items-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100"
                    >
                        <span className="font-bold">X</span> Cancelar
                    </button>
                    <button
                        type="submit"
                        disabled={isPending}
                        className="flex items-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                        </svg>
                        {isPending ? 'Guardando...' : 'Guardar'}
                    </button>
                </div>
            </form>
        </BottomSheet>
    );
}
